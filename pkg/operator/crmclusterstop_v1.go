...
// # Details
//
// This operator is designed to safely stop a CRM cluster, ensuring that the cluster is only stopped if it is online and the provided cluster ID matches the system's cluster ID.
...
func WithCustomClusterClientStop(clusterClient cluster.Cluster) CrmClusterStopOption {
